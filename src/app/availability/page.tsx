...
const FALLBACK_TZS = [
  "Asia/Colombo","Asia/Kolkata","Asia/Dubai","Asia/Singapore","Asia/Kuala_Lumpur",
  "Asia/Shanghai","Asia/Tokyo","Asia/Manila","Asia/Jakarta",
...
export default function AvailabilityPage(){
  const [week, setWeek] = useState<Week>(() => [...Array(7)].map((_,i)=>({day:i, slots:[]})));
  ...
  useEffect(() => {
    const run = async () => {
      try {
        const res = await fetch("/api/availability", { headers: { "x-user-id": "DEMO_USER" } });
        if (!res.ok) throw new Error(`GET /api/availability failed: ${res.status} ${await res.text()}`);
    ...
    function handleUp() {
      ...
      const endMinFromHover =
        lastHover.current && lastHover.current.day === day
          ...
          : startMin;
      const end = endMinFromHover === startMin ? startMin + STEP : endMinFromHover + STEP;
  ...
  const rows = useMemo(() => Array.from({length: Math.floor(1440/STEP)}, (_,i)=>i*STEP), []);
  ...
  function addSlot(day:number, startMin:number, endMin:number){
    ...
    setWeek(prev => prev.map((d: Day) => {
      if (d.day !== day) return d;
      const merged = mergeSlots([...d.slots, { startMin: clampedStart, endMin: clampedEnd }]);
  ...
  function removeSlot(day:number, idx:number){
    setWeek(prev => prev.map((d: Day) => d.day===day ? ({...d, slots: d.slots.filter((_,i)=>i!==idx)}) : d));
  }
  ...
  const isCellInPreview = (day: number, min: number): boolean => {
    ...
    const hover = lastHover.current?.day === day ? lastHover.current.min : start;
    const a = Math.min(start, hover);
    const b = Math.max(start, hover) + STEP;
    const cellStart = min, cellEnd = min + STEP;
  ...
  async function saveNow(){ // manual save (button)
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          {saveState === "saved" && <span className="text-green-600 text-sm">Saved ✓</span>}
          {saveState === "error" && <span className="text-red-600 text-sm">Save failed</span>}
        </div>
      </div>
      ...
      <div className="overflow-auto border rounded-2xl select-none">
        <div className="min-w-[900px] grid" style={{ gridTemplateColumns: "80px repeat(7, 1fr)" }}>
          {/* header row */}
          <div />
          {DAY_LABELS.map(d => <div key={d} className="p-2 font-semibold text-center">{d}</div>)}

          {/* time rows */}
          {rows.map((min) => (
            <React.Fragment key={`row-${min}`}>
              <div className="border-t p-1 text-xs text-right pr-2">{fmt(min)}</div>
              {week.map((d) => {
                ...
                return (
                  <div
                    ...
                    className={`border-t border-l h-6 cursor-crosshair transition-colors
                      ${preview ? "bg-green-300/70" : saved ? "bg-green-200" : "hover:bg-gray-100"}`}
        ...
        {week.map((d) => (
          <div key={`day-${d.day}`} className="p-3 rounded-2xl border">
            ...
            <div className="flex flex-wrap gap-2">
              {d.slots.length === 0 && <span className="text-sm opacity-70">No free slots</span>}
              {d.slots.map((s, idx) => (
                <span key={`${s.startMin}-${s.endMin}`} className="text-sm px-2 py-1 rounded-full bg-green-100 border">
                  {fmt(s.startMin)}–{fmt(s.endMin)}
                  <button className="ml-2 text-red-600" onClick={()=>removeSlot(d.day, idx)}>×</button>
                </span>
              ))}
            </div>
          </div>
